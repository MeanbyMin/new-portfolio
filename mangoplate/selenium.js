...
(async function example() {
  ...
  try {
    ...
    // 리스트 페이징
    let paging = await driver.findElements(
      By.css(".paging-container .paging a")
    );
    for (let i = 2; i < paging.length; i++) {
      await paging[i].click();
      ...
      for (let i = 0; i < 20; i++) {
        ...
        try {
          ...
          let repphoto1 = repphoto.split("?")[0].trim();
          r_repphoto = await repphoto1;
        } catch {}
        ...
        r_repadd = await (
          await (await repadd[i].getText()).split("-")[0]
        ).trim();
        ...
        // 태그명 찾기
        let restaurant_tags = await driver.findElements(
          By.className("tag-item")
        );
        ...
        // 사진 리스트 찾아서 클릭하기
        try {
          let owl_wrapper = await driver.findElement(
            By.className("owl-wrapper")
          );
          ...
          let close_icon = await driver.findElement(By.className("close_icon"));
          await close_icon.click();
        } catch {}

        try {
          let imageArea = await driver.findElement(
            By.css(".pg-restaurant .restaurant-detail .info .list-thumb-photos")
          );
          let menuimg = await imageArea.findElement(
            By.className("center-croping")
          );
          // imageArea가 있다면 클릭
          await menuimg.click();
          ...
          try {
            await pool.getConnection(function (err) {
              ...
              try {
                let param = resultarr;
                let sql =
                  "INSERT INTO mango_restaurant (r_writer, r_restaurant, r_branch, r_grade, r_repphoto, r_repadd, r_address, r_jibunaddress, r_tel, r_foodtype, r_price, r_parking, r_openhour, r_breaktime, r_lastorder, r_holiday, r_website, r_menu, r_menuprice, r_menuphoto, r_photo, r_tags, r_status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
                pool.query(sql, param, function (err, result) {
                  if (err) throw err;
                  console.log("DB 추가 완료");
                });
              } catch {
                console.log("DB 추가 실패");
              }
            });
          } catch {
            console.log("DB 추가 실패");
