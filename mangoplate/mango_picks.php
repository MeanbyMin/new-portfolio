...
<?php
        ...
        <main class="pg-all_picks">
            <article class="contents">
                        ...
                        <ul class="list-represented-picks">
                            <li>
                                ...
                                <figure class="mango-pick-item">
                                    <div class="thumb"
                                        style="background-image: url(./img/mango_picks/5rh5cjpxshg5kw7o.png)">
                                    </div>
                                    <figcaption>
                                    <strong class="title">테이크아웃 스시 맛집 6곳</strong>
                                    <p class="desc" class="
                                        ">
                                        집에서 즐기는 스시 클라스가 이정도!
                                    </p>
                ...
                <!-- 에디터 망고픽 -->
<?php
    $sql = "SELECT ms_idx, ms_userid, ms_userphoto, ms_title, ms_subtitle, ms_intro, ms_repphoto FROM mango_story WHERE ms_status ='등록' ORDER BY ms_idx DESC LIMIT 4";
    $result = mysqli_query($conn, $sql);
    $story_lists = [];
    while($row = mysqli_fetch_array($result)){
        $storyadd = array('ms_idx' => $row['ms_idx'], 'ms_userid' => $row['ms_userid'], 'ms_userphoto' => $row['ms_userphoto'], 'ms_title' => $row['ms_title'], 'ms_subtitle' => $row['ms_subtitle'], 'ms_intro' => $row['ms_intro'], 'ms_repphoto' => $row['ms_repphoto']);
...
?>
                        ...
                        <li class="mango-pick-item-content-wrap">
                            <a onclick="" href="./mango_picks_detail.php?ms_idx=<?=$story_lists[$i]['ms_idx']?>">
                            <figure class="mango-pick-item">
                                <div class="thumb lazy" style="background-image: url(<?=$story_lists[$i]['ms_repphoto']?>);">
                                </div>
                                ...
                                <div class="thumb">
                                    <img class="center-crop lazy"
                                        alt="" src="<?=$story_lists[$i]['ms_userphoto']?>">
                                </div>
                            ...
                            <div class="review-content">
                                <figure class="user">
                                <figcaption>
                                    <?=$story_lists[$i]['ms_userid']?>
                                </figcaption>
                                </figure>
                                <p>
                                    <?=$story_lists[$i]['ms_intro']?>
                                </p>
                            </div>
                            </a>
                        </li>
