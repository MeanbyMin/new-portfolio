  ...
  xhr.onload = () => {
    if (xhr.status === 200) {
      ...
      for (let i = 0; i < review.length; i++) {
        const listEditorsPick = document.querySelector(".list-editors-pick");
        let li = document.createElement("li");
        li.setAttribute("class", "mango-pick-item-content-wrap");
        li.innerHTML = `
        <a onclick="" href="./mango_picks_detail.php?ms_idx=${review[i][0]}">
          <figure class="mango-pick-item">
              <div class="thumb lazy" style="background-image: url(${review[i][6]});">
              </div>

              <figcaption>
                  <strong class="title">${review[i][3]}</strong>
              ...
              <figcaption>${review[i][1]}</figcaption>
              </figure>
          </figure>
