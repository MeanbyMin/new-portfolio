$(document).ready(function() {
    $.ajax({
        ...
        $(e).find("channel item").each(function() {
            let e = $(this).find("title").text()
              , t = $(this).find("link").text()
              , i = ($(this).find("description"),
            $(this).find("pubDate").text())
              , n = new Date(i)
              , a = n.getFullYear() + "." + (n.getMonth() + 1) + "." + n.getDate()
              , l = (new DOMParser).parseFromString($(this).find("description").text(), "text/html")
              , d = (l.getElementsByTagName("img"),
            ...
            r.style.backgroundImage = `url('${l.getElementsByTagName("img")[0].src}')`,
